...
Interval at **`r strftime( strptime( paste(Sys.Date(), sprintf('%04d', mostActiveInterval)), format="%Y-%m-%d %H%M" ), '%H:%M' )`** contains `r round(stepsPerInterval[which.max(stepsPerInterval$meanStepsPerInterval),]$meanStepsPerInterval)` steps on average.
